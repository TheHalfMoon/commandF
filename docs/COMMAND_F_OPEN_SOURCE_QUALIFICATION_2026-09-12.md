...
| [inferno-framework/client-fhir-testing](https://github.com/inferno-framework/client-fhir-testing) | DEPRECATED_OR_ARCHIVED | RESEARCH_ONLY | historical CF-19/20/22 research | GitHub repository metadata reports `archived=true`; retain only as historical recorded-transaction -> validation/replay prior art. Do not start a new dependency. Any study must pin an immutable ref and preserve the privacy/data-boundary caveats. |
