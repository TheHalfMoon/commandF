...
fn write_baseline(
    ...
    assert_eq!(output.status.code(), Some(2));
    fs::write(path, output.stdout).expect("write baseline report");
}
...
fn historical_baseline_allows_same_semantic_finding() {
    ...
    let baseline_dir = dir.join("baseline-state");
    let current_dir = dir.join("current-state");
    let (base_before_lock, base_before_cache, base_after_lock, base_after_cache) =
        changed_states(&baseline_dir, "0.8.0", "0.9.0");
    let (before_lock, before_cache, after_lock, after_cache) =
        changed_states(&current_dir, "1.0.0", "1.1.0");
...
fn exact_suppression_passes_and_stale_suppression_does_not_hide_blocker() {
    ...
    let fingerprint = finding_fingerprint_v1(
